...
def refine_predict(
    ...
    gpu_ids : str
        the GPU ids of the machine to use. If empty string, will try to use GPU first.
        If only single GPU, use: "0,". Use "cpu" for CPU inference.
    modulo : int
    ...
    # For CPU inference, put everything on CPU
    if devices[0].type == "cpu":
        resblocks_per_gpu = n_resnet_blocks
    else:
        resblocks_per_gpu = n_resnet_blocks // len(devices)
    ...
    if devices[0].type == "cpu":
        forward_rears.append(inpainter.generator.model[first_resblock_ind:])
        forward_rears[0].to(devices[0])
    else:
        for idd in range(len(devices)):
            if idd < len(devices) - 1:
